...
/** Map filter condition value to product.condition */
...
function HomeContent() {
  ...
  const displayedProducts = useMemo(() => {
    ...
    if (activeCategory !== 'Sve') {
      base = dbProducts.filter((p: Product) => p.category === activeCategory);
    }

    // Location filter
    if (selectedLocation) {
      base = base.filter((p: Product) => p.location === selectedLocation.name);
    }
    ...
    // Price filter — combines manual filter modal + AI-parsed range
    const effectivePriceMin = aiPriceMin ?? (filters.priceMin ? Number(filters.priceMin) : undefined);
    const effectivePriceMax = aiPriceMax ?? (filters.priceMax ? Number(filters.priceMax) : undefined);
    if (effectivePriceMin && !isNaN(effectivePriceMin)) base = base.filter((p: Product) => p.price >= effectivePriceMin);
    if (effectivePriceMax && !isNaN(effectivePriceMax)) base = base.filter((p: Product) => p.price <= effectivePriceMax);

    // Condition filter
    if (filters.condition !== 'all') {
      const allowed = CONDITION_MAP[filters.condition] || [];
      if (allowed.length > 0) base = base.filter((p: Product) => allowed.includes(p.condition));
    }
    ...
    if (filters.sortBy === 'price_asc') {
      ...
      base = [...base].sort((a, b) => (b.views || 0) - (a.views || 0));
    }

    return base;
  }, [dbProducts, activeCategory, searchQuery, selectedLocation, filters, aiPriceMin, aiPriceMax]);
            ...
            <div className="relative w-full max-w-4xl bg-[var(--c-card)] border border-[var(--c-border)] rounded-[6px] shadow-2xl animate-fadeIn flex flex-col" style={{ height: 'min(90vh, 700px)' }}>
              ...
              <div className="flex-1 flex flex-col px-4 md:px-6 py-4 md:py-5 gap-4 md:gap-5 min-h-0 overflow-y-auto">
                ...
                <div className="flex items-center gap-3 shrink-0">
                  ...
                  <p className="text-[9px] font-black text-[var(--c-text3)] uppercase tracking-[0.25em]">Šta AI pretraga razumije?</p>
                </div>
                ...
                <div className="shrink-0">
                  <div className="flex items-center gap-3 mb-3">
                    <div className="w-8 h-[2px] bg-purple-500"></div>
                    <p className="text-[9px] font-black text-[var(--c-text3)] uppercase tracking-[0.25em]">Probaj odmah — klikni primjer</p>
                  </div>
                ...
                <div className="flex flex-col md:flex-row gap-4 md:gap-6 shrink-0">
                  ...
                  <div className="hidden md:flex gap-3 shrink-0">
                    <div className="w-36 bg-[var(--c-hover)] border border-[var(--c-border)] rounded-[4px] p-4">
                      ...
                      <p className="text-[8px] font-bold text-[var(--c-text3)] uppercase tracking-widest">AI Monitoring</p>
                    </div>
                    <div className="w-36 bg-[var(--c-hover)] border border-[var(--c-border)] rounded-[4px] p-4">
                      ...
                      <p className="text-[8px] font-bold text-[var(--c-text3)] uppercase tracking-widest">Bit Enkripcija</p>
                    </div>
                  </div>
                </div>
        ...
        <div className="sticky top-16 md:top-20 z-30 bg-[var(--c-glass)] backdrop-blur-xl -mx-4 px-4 pt-2 md:pt-3 pb-0 border-b border-gray-200/80 shadow-sm transition-all">
          ...
          <div className="flex items-center justify-end mb-2 px-1">
            <button
              ...
              className="flex items-center gap-1.5 text-[9px] font-bold text-gray-400 hover:text-blue-500 transition-colors"
            >
              <i className="fa-solid fa-shield-halved text-[9px]" aria-hidden="true"></i>
          ...
          <div className="flex justify-center mb-3">
            <div className="flex gap-2 w-full max-w-[480px]">
              ...
              <div className="relative flex-1 group">
                <div className="absolute inset-0 bg-blue-50/50 rounded-[20px] blur-md group-hover:bg-blue-50 transition-colors"></div>
                {isAiSearching
                  ? <i className="fa-solid fa-spinner animate-spin absolute left-5 top-1/2 -translate-y-1/2 text-blue-500 text-[12px] z-10"></i>
                  : <i className="fa-solid fa-magnifying-glass absolute left-5 top-1/2 -translate-y-1/2 text-gray-400 text-[12px] group-focus-within:text-blue-500 transition-colors z-10"></i>
                }
                <input
                  type="text"
                  role="combobox"
              ...
              <div className="flex gap-2.5 px-1 md:justify-center min-w-max">
                ...
                >
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center gap-1.5">
                    <div className="w-8 h-8 rounded-full bg-white/15 flex items-center justify-center backdrop-blur-md border border-white/15 shadow-lg">
                ...
                <div className="flex flex-1 overflow-hidden border-t border-gray-100">
                  ...
                  {!showThirdLevel && (
                    <div className="flex-1 bg-white overflow-y-auto p-4 md:p-6 pb-24 relative">
                      ...
                      <div className="mb-6 flex items-center gap-3 relative z-10">
                        <div className="w-12 h-12 rounded-[14px] bg-gradient-to-br from-blue-600 to-indigo-600 flex items-center justify-center text-white shadow-md shadow-blue-500/20">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-2 animate-[fadeIn_0.2s_ease-out]">
                        ...
                        <button
                          ...
                          className="col-span-full text-left bg-gradient-to-r from-blue-600 to-blue-500 text-white p-3.5 rounded-[12px] flex justify-between items-center shadow-lg shadow-blue-500/20 mb-1 hover:scale-[1.01] active:scale-[0.99] transition-all group"
                        >
                          <span className="text-[11px] font-black uppercase tracking-wider">Prikaži sve u {selectedCategory.name}</span>
                  ...
                  {showThirdLevel && selectedSubGroupData && (
                    <div className="flex-1 bg-white overflow-y-auto p-4 md:p-6 pb-24 relative animate-[fadeIn_0.2s_ease-out]">
                      ...
                      <button
                        onClick={() => { handleCategoryChange(selectedCategory.name); setShowAllCatsPopup(false); setSelectedSubGroup(null); }}
                        className="w-full text-left bg-gradient-to-r from-indigo-600 to-blue-500 text-white p-3.5 rounded-[12px] flex justify-between items-center shadow-md shadow-indigo-500/20 mb-3 hover:scale-[1.01] active:scale-[0.99] transition-all group"
                      >
                        <span className="text-[11px] font-black uppercase tracking-wider">Prikaži sve u {selectedSubGroup}</span>
                        <div className="w-6 h-6 bg-white/20 rounded-full flex items-center justify-center">
                          <i className="fa-solid fa-arrow-right text-[10px]"></i>
                        </div>
                      </button>
          ...
          <div className="pt-6 pl-1 md:pl-0">
            ...
            <div className="flex overflow-x-auto no-scrollbar gap-3 pb-4 pr-4">
              {vehicleProducts.slice(0, 10).map((p) => (
                <div key={`vozila-${p.id}`} className="min-w-[150px] w-[150px] shrink-0 cursor-pointer" onClick={() => handleProductClick(p.id)}>
        ...
        <div className="pt-2">
          <div className="flex justify-between items-end mb-4 px-1">
            ...
            >
              ...
              Filteri
              {activeFilterCount > 0 && (
                <span className="w-4 h-4 rounded-full bg-blue-500 text-white text-[8px] font-black flex items-center justify-center -mr-0.5">
                  {activeFilterCount}
                </span>
              )}
            </button>
